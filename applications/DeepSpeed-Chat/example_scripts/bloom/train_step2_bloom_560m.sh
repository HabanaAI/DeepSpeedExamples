...
tensorboard_enabled=${HL_TENSORBOARD_ENABLED:-0}
...
if [ "$lora_dim" -ne "0" ]; then
  lora_args="--lora_dim ${lora_dim} --lora_learning_rate ${lora_learning_rate} --lora_module_name rwtransformer.h. --only_optimize_lora "
...
tb_enabled_arg=""
if [ "$tensorboard_enabled" -eq "1" ]; then
  tb_enabled_arg="--enable_tensorboard "
fi
